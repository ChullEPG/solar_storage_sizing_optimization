...
def objective_function(x, a):
    ...
    total_capital_cost = economic_analysis.calculate_pv_capital_cost(pv_capacity, battery_capacity, a,  linearize_inverter_cost = a['linearize_inverter_cost'])
...
def objective_function_with_solar_and_battery_degradation(x, a):
    ...
    # Battery repurchase cost (always 0 unless otherwise specified)
    ...
    for year in tqdm(range(a['Rproj']), desc = "Optimizing", ncols=100, bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}"):
        ...
        cash_flows.append(energy_savings_per_year + operational_savings_per_year + carbon_savings_per_year - maintenance_costs - battery_repurchase_cost)
...
# With loan
def objective_function_with_solar_and_battery_degradation_loan_v1(x, a):
    ...
    for year in tqdm(range(a['Rproj']), desc="Optimizing", ncols=100, bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}"):
      ...
      #  loadshedding_schedule = generate_data.generate_loadshedding_schedule(pv_output_profile, a['loadshedding_probability'])
        ...
        # Operational
        operational_savings_per_year =0 #value_of_charging_saved_by_pv_from_loadshedding.sum() # (float) revenue per year from saved passengers
