...
def objective_function_with_solar_and_battery_degradation_loan_v2(x, a):
    ...
    ## Annual payments
    annual_pv_payment = economic_analysis.get_crf(a) * pv_capital_cost
    annual_bss_payment = economic_analysis.get_crf(a) * battery_capital_cost
    
    annual_payment = annual_pv_payment + annual_bss_payment
    ...
    battery_repurchase_cost = 0 # Initialize battery repurchase cost 
    ...
    for year in tqdm(range(a['Rproj']), desc="Optimizing", ncols=100, bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}"):
        ...
        net_cash_flows.append(energy_savings_per_year - annual_payment - cost_of_trickle_charging - maintenance_costs - battery_repurchase_cost)
