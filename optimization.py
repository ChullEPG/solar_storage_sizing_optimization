...
def objective_function_with_incremental_fleet_electrification(x, a):
    ...
    for year in tqdm(range(a['Rproj']), desc="Optimizing", ncols=100, miniters = 0, bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}", disable = False):
    #for year in range(a['Rproj']):
...
def objective_function_with_changed_load_shedding(x, a):
    ...
    #for year in range(a['Rproj']):
        if year % 2 == 0:
            a['load_shedding_schedule'] = a['load_shedding_schedule_active']
            a['load_profile'] = a['load_profile_4']
        else:
            a['load_shedding_schedule'] = a['load_shedding_schedule_inactive']
            a['load_profile'] = a['load_profile_ls_1']
